    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (studentsJson.success) {
          setStudents(studentsJson.data.map((s: any) => ({
            id: s.auth_id,
            internalId: s.id,
        ...
        if (recruitersJson.success) {
          setRecruiters(recruitersJson.data.map((r: any) => {
            let daysLeft = "N/A";
      ...
      <main className="flex-1 p-4 md:p-6 overflow-y-auto">
        <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
          <TabsList className="w-full grid grid-cols-3 bg-[hsla(216,36%,18%,1)] border border-border/30 h-12 mb-6">
            <TabsTrigger value="students" className="gap-2 data-[state=active]:bg-foreground/30 data-[state=active]:text-primary-foreground">
              <Users className="h-4 w-4" />
              Students
            </TabsTrigger>
            <TabsTrigger value="recruiters" className="gap-2 data-[state=active]:bg-foreground/30 data-[state=active]:text-primary-foreground">
              <Users className="h-4 w-4" />
              Recruiters
            </TabsTrigger>
            <TabsTrigger value="enterprise" className="gap-2 data-[state=active]:bg-foreground/30 data-[state=active]:text-primary-foreground">
              <Users className="h-4 w-4" />
              Enterprise
            </TabsTrigger>
          </TabsList>
